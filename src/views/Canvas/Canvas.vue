...
<template>
  <div class="container">
    <div class="input-wrap">
      <Input class="iInput" v-model.trim="input" placeholder="请输入内容" />
      <Button type="primary"  @click="changeShow">确定</Button>
    </div>
    <div class="box">
...
export default {
  ...
  created() {
    this.$store.commit("changeRouterIndex", 7);
  },
  ...
  methods: {
    ...
    shape() {
      ...
      // 说明: 由于imageData中的data数组，表示的是图片数据的每个点的r/g/b/a的值，每个值都是0-255（buffer是8位的），
      // 即第一个数据表示第一个像素点的r值，第二个数据表示第一个像素点的g值，以此类推，所以这里4个数据合在一起才是一个完整的rgba值，才能表示一个像素点。
      // 所以我们将这个8位的数据，转为32位数据，即将这里四个数据合成一个32位数据，因此得到的32位数据数组，每个元素刚好对应图片上一个像素点。
      // console.log('__data32',data32);
      let posArr = []; // 有效抽样点的集合
      for (let y = 0; y < this.height; y += this.grid) {
        // 每隔posDistance行循环一次，取y值
        for (let x = 0; x < this.width; x += this.grid) {
          // 每行中，每隔posDistance列循环一次，取x值
          if (data32[y * this.width + x]) {
            // 如果第y行第x个点数据不为0，即改点存在色值
            posArr.push([x, y]);
          }
        }
      }
      ...
      function Point([x, y]) {
        ...
        this.color =
          $vue.point.colors[
            Math.floor(
                Math.random() * ($vue.point.colors.length + 2)
            )
          ];
      }
    ...
    flyAway() {
      ...
      this.oldPoints.forEach((p, i) => {
        ...
        if (!p.speedX && !p.speedY) {
          ...
          // 根据速度和x方向的速度，计算y方向速度：
          p.speedY = Math.sqrt(
            Math.pow(speed, 2) - Math.pow(p.speedX, 2)
          );
          // 方向随机（正负号）：
